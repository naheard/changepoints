...
def simulate_changepoint_data(n=1000,p=2,tau=[],regimes=[0],num_categories=np.array([3,5],dtype=int),inclusion_ps=np.repeat(0.3*0+1,2),seed=None,y_filename=None,x_filename=None):
    if seed is not None:
        np.random.seed(seed)
    n_cps=len(tau)
...
def main():
    ...
    for i in range(len(probability_models)):
        ...
        simulate_changepoint_data(n=n,y_filename="y"+file_ending,x_filename="x"+file_ending,seed=s,tau=cps,regimes=regimes)
