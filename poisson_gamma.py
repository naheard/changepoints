...
class PoissonGamma(ProbabilityModel):
    def __init__(self,data=None,alpha_beta=(.1,.1),T=1):
        ...
        self.T=1
        self.lhd_constant=-gammaln(self.a0)
    ...
    def likelihood_j(self,j=0,start_end=[(0,None)],y=None):
        ...
        lhd_terms_zero=self.a0*np.log(self.b0/(self.b0+float(n)))
        lhd=lhd_terms_zero+self.lhd_constant+(0 if r==0 else (gammaln(self.a0+r)-r*np.log(self.b0+n)))
        return(lhd)
